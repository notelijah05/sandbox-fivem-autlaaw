...
export default ({ alert }) => {
	...
	const meAttached = alert.attached.includes(
	...
	const getIcon = () => {
		switch (alert.style) {
			case 1:
				return <FontAwesomeIcon icon={['fas', 'bell']} />;
			case 2:
			...
			<div className={`${classes.wrapper} type-${alert.style}${alert.panic ? ' panic' : ''}`}>
				<Grid container>
					...
					>
						<div style={{ display: 'flex', alignItems: 'center' }}>
							{meAttached && (
								<FontAwesomeIcon size="xs" icon={['fas', 'thumbtack']} style={{ marginRight: 10 }} />
							)}
							{getIcon()}
							...
							{hasAttaching && (
								<div>
									<IconButton
										onClick={() => {
											const myCallsign =
												myUnitData.operatingUnder !== null
													? myUnitData.operatingUnder
													: myUnitData.primary;
											if (meAttached) {
									...
									<Autocomplete
										...
										renderInput={(params) => (
											<TextField
												...
												endadornment={null}
												helperText={
													unitJobCountText.length > 0 ? `Numbers: ${unitJobCountText}` : null
												}
												FormHelperTextProps={{
													sx: {
														marginTop: 0,
										...
										renderTags={(value, getTagProps) => {
											const myCallsign = myUnitData?.operatingUnder
												? myUnitData?.operatingUnder
												: myUnitData?.primary;
											...
											return value.map((option, index) => {
												return (
													<Chip
														...
														variant="outlined"
														label={`${
															option === myUnitData.primary ||
															option === myUnitData.operatingUnder
																? '[You] '
																: ''
														}${option} ${
															allSubUnits.filter((u) => u.operatingUnder === option)
																?.length > 0
																? `(+${
																		allSubUnits.filter(
																			(u) => u.operatingUnder === option,
																		)?.length
																  })`
																: ''
														}`}
						...
						>
							...
							{Boolean(alert.camera) && (
								<IconButton onClick={onCamera} style={{ marginLeft: 'auto' }}>
									<FontAwesomeIcon size="2xs" icon={['fas', 'video']} />
								</IconButton>
							)}
						</Grid>
					)}
				...
				>
					{alert.attached?.length > 0 ? (
						<p>
							Are you sure you want to clear this call, there are {alert.attached.length} unit(s) still
							attached, it will be removed from the entire dispatch system.
						</p>
					) : (
						<p>
							Are you sure you want to clear this call, it will be removed from the entire dispatch
							system?
						</p>
					)}
				</Modal>

...
import { Grid, TextField, InputAdornment, IconButton, Pagination, List, MenuItem, Fab, Checkbox } from '@mui/material';
...
export default () => {
	...
	const [reportType, setReportType] = useState(
		ReportTypes.find((r) => hasPermission(r.requiredViewPermission))?.value ?? 1,
	);
	...
	useEffect(() => {
		if (qry.search) {
			...
			const availTypes = ReportTypes.filter(
				(r) => hasPermission(r.requiredViewPermission) || (r.allowAttorney && isAttorney),
			);
	...
	const fetch = useMemo(
		() =>
			throttle(async (value, rType, page, currentPages, evidenceMode) => {
				let s = qs.parse(location.search.slice(1));
				s.search = value;
				history({
					path: location.pathname,
	...
	return (
		<>
						...
						<Grid container spacing={1}>
							...
							<Grid item xs={4}>
								...
								>
									{ReportTypes.filter(
										(r) =>
											hasPermission(r.requiredViewPermission) || (r.allowAttorney && isAttorney),
									).map((option) => (
							...
							<Grid item xs={7}>
								<TextField
									...
									label={
										toggleEvidenceMode ? 'Search Reports By Evidence Identifier' : 'Search Reports'
									}
									InputProps={{
										endAdornment: (
											<InputAdornment position="end">
												{searchTerm != '' && (
													<IconButton size="small" type="button" onClick={onClear}>
					...
					{loading ? (
						...
						<>
							<List className={classes.items}>
								{results.map((result) => {
									return <Item key={result._id} report={result} />;
								})}
							</List>
						</>
					) : (
			...
			{myJob && (
				<Fab color="primary" className={classes.fab} onClick={createReport}>

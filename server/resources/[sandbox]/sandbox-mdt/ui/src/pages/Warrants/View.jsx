...
import { List, ListItem, ListItemText, Grid, ButtonGroup, Button, TextField, MenuItem } from '@mui/material';
		...
		return (
			<div className={classes.nWrapper}>
				{loading ? (
					...
					<>
						<FontAwesomeIcon className={classes.nWrapperIcon} icon={['fas', 'file']} />
		...
		<div className={classes.wrapper}>
			...
			>
				{myJob?.Id === 'police' && (
					<Grid item xs={12}>
						<ButtonGroup fullWidth>
							<Button onClick={() => setUpdating(warrant.state === 'active' ? 'void' : warrant.state)}>
								Update Status
							</Button>
						</ButtonGroup>
					</Grid>
				...
				<Grid item xs={6}>
					<List>
						<ListItem>
							<ListItemText primary="Title" secondary={warrant.title} />
						</ListItem>
						<ListItem>
							<ListItemText primary="Issued At" secondary={moment(warrant.issued).format('LLLL')} />
						</ListItem>
						{warrant.suspectData && (
							<ListItem>
								<ListItemText
									...
									<ListItemText
										...
										secondary={
											<Link
												...
												to={`/reports?report=${warrant.report}&mode=view`}
											>
						...
						{warrant.status !== 'active' ? (
							...
							<ListItem>
								<ListItemText primary="Expires" secondary={moment(warrant.expires).format('LLLL')} />
							</ListItem>
						) : (
							<ListItem>
			...
			>
				<p>
					A warrant state cannot be changed back to active once changed, a new warrant will have to be issued.
				</p>
				<TextField
					select
					fullWidth

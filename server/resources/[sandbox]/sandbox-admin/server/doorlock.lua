...
function RegisterDoorlockCallbacks()
    exports["sandbox-base"]:RegisterServerCallback('Admin:GetDoorLocks', function(source, data, cb)
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:GetDoorLockById', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:CreateDoorLock', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end
        ...
        MySQL.insert('INSERT INTO ox_doorlock (name, data) VALUES (?, ?)', { doorData.name, encodedData },
            function(insertId)
                if insertId then
                    ...
                    TriggerClientEvent('sandbox-hud:Notification', source, 'success', 'Door created successfully!', 3000)
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:UpdateDoorLock', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end
        ...
        MySQL.update('UPDATE ox_doorlock SET name = ?, data = ? WHERE id = ?', { doorName, encodedData, id },
            function(affectedRows)
                if affectedRows and affectedRows > 0 then
                    ...
                    TriggerClientEvent('sandbox-hud:Notification', source, 'success', 'Door updated successfully!', 3000)
                    cb({ success = true })
                else
                    cb({ success = false })
                end
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:DeleteDoorLock', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end
        ...
        MySQL.update('DELETE FROM ox_doorlock WHERE id = ?', { doorId }, function(affectedRows)
            if affectedRows and affectedRows > 0 then
                ...
                TriggerClientEvent('sandbox-hud:Notification', source, 'success', 'Door deleted successfully!', 3000)
                cb({ success = true })
            else
                cb({ success = false })
            end
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:TeleportToDoorLock', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:ToggleDoorLockState', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end
    ...
    exports["sandbox-base"]:RegisterServerCallback('Admin:GetDoorLockSounds', function(source, data, cb)
        if not hasAdminPermission(source) then
            return cb(false)
        end

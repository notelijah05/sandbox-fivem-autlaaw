...
RegisterNUICallback('GetDoorLocks', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:GetDoorLocks', data, cb)
end)

RegisterNUICallback('GetDoorLockById', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:GetDoorLockById', data, cb)
end)

RegisterNUICallback('CreateDoorLock', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:CreateDoorLock', data, function(success)
        if success then
            cb({
                success = true,
...
RegisterNUICallback('UpdateDoorLock', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:UpdateDoorLock', data, function(success)
        if success then
            cb({
                success = true,
...
RegisterNUICallback('DeleteDoorLock', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:DeleteDoorLock', data, function(success)
        if success then
            cb({
                success = true,
...
RegisterNUICallback('TeleportToDoorLock', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:TeleportToDoorLock', data, function(success)
        if success then
            cb({
                success = true,
...
RegisterNUICallback('ToggleDoorLockState', function(data, cb)
    exports["sandbox-base"]:ServerCallback('Admin:ToggleDoorLockState', data, function(success)
        if success then
            cb({
                success = true,
        ...
        while isAddingDoorlock do
            ...
            if hit and entity > 0 and GetEntityType(entity) == 3 and (doorCount == 1 or doorA ~= entity) then
                ...
                if IsDisabledControlJustPressed(0, 24) then
                    ...
                    exports['sandbox-hud']:Notification("success", string.format("Door %d selected", #tempData), 3500)
                end
            end

            if IsDisabledControlJustPressed(0, 25) then
                isAddingDoorlock = false
                exports['sandbox-hud']:ActionHide("doorlock_add")
                ...
                exports['sandbox-hud']:Notification("info", "Door selection cancelled", 3500)
            ...
            if #tempData >= doorCount then
                ...
                exports['sandbox-hud']:Notification("success", "Doors selected! Fill in the details.", 3500)
...
RegisterNUICallback('GetClosestDoorLock', function(data, cb)
    ...
    if closestDoor then
        exports["sandbox-base"]:ServerCallback('Admin:GetDoorLockById', { id = closestDoor.id }, cb)
    else
        cb(false)
    end
end)
...
RegisterNetEvent('sandbox-admin:openDoorlockMenu', function()
    if LocalPlayer.state.isStaff then

...
function registerUsables()
	exports['sandbox-inventory']:RegisterUse("wine_bottle", "Status", function(source, itemData)
		...
		if currentMeta.GlassesRemaining >= 1 then
			...
			currentMeta = exports['sandbox-inventory']:UpdateMetaData(itemData.id, currentMeta)
			exports['sandbox-inventory']:AddItem(itemData.Owner, "wine_glass", 1, {}, 1)
		else
	...
	for k, v in pairs(alcoholItems) do
		exports['sandbox-inventory']:RegisterUse(k, "Status", function(source, itemData)
	...
	exports['sandbox-inventory']:RegisterUse("scuba_gear", "Status", function(source, slot, itemData)
